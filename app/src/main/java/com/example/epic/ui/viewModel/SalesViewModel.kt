...
class SalesViewModel @Inject constructor(private val repository: SalesRepository) : ViewModel() {
    ...
    private val _listSalesResponse = MutableLiveData<NetworkResult<ListSalesResponse>>()
    val listSalesResponse: LiveData<NetworkResult<ListSalesResponse>>
        get() = _listSalesResponse

    private val _createSalesResponse = MutableLiveData<NetworkResult<CreateSalesResponse>>()
    val createSalesResponse: LiveData<NetworkResult<CreateSalesResponse>>
        get() = _createSalesResponse

    private val _deleteSalesResponse = MutableLiveData<NetworkResult<DeleteSalesResponse>>()
    ...
    fun listSalesRequest() {
        viewModelScope.launch {
            val connected = CheckInternet().check()
            if (connected) {
                _listSalesResponse.postValue(NetworkResult.Loading())
                _listSalesResponse.postValue(repository.readSales())
            }else{
